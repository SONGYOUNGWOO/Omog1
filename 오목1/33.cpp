...
public:
    ...
    void check_overlap(int x, int y) {
        // 입력된 위치가 보드 내에 있는지 확인
        if (x < 0 || x >= BOARD_SIZE || y < 0 || y >= BOARD_SIZE) {
            cout << "입력된 위치가 보드 범위를 벗어났습니다. 다시 입력하세요." << endl;
            enter_location();
            return; // 잘못된 위치이므로 함수 종료
        }

        // 입력된 위치가 비어 있는지 확인
        if (board[y][x] != '-') { // x는 가로, y는 세로
            cout << "다른 위치를 입력하십시오. 해당 위치는 이미 점유되어 있습니다." << endl;
            enter_location();
            return; // 이미 점유된 위치이므로 함수 종료
        }
    }
    ...
    void saveGame(const string& filename) {
        ...
        if (file.is_open()) {
            file << black_count << " " << white_count << endl;
            for (int i = 0; i < BOARD_SIZE; ++i) {
                for (int j = 0; j < BOARD_SIZE; ++j) {
                    file << board[i][j] << " ";
                }
            ...
            file.close();
            cout << "게임 상태가 저장되었습니다: " << filename << endl;
        }
        else {
    ...
    void loadGame(const string& filename) {
        ...
        if (file.is_open()) {
            ...
            file.close();
            cout << "게임 상태가 불러와졌습니다: " << filename << endl;
        }
        else {
            cout << "파일을 열 수 없습니다: " << filename << endl;
        }
...
int main() {
    ...
    char choice;
    cout << "랜덤으로 돌을 배치하려면 'h'를 입력하고, 저장된 게임을 불러오려면 'l'을 입력하세요: ";
    cin >> choice;
    if (choice == 'h') {
    ...
    while (k < BOARD_SIZE * BOARD_SIZE && !end) {
        ...
        cout << "위치를 입력하거나 's'를 입력하여 게임을 저장하세요.\n";
        cout << "x (0-" << BOARD_SIZE - 1 << "): ";
        string input;
        cin >> input;
        if (input == "s") {
