    ...
    void countAndPrintMaxConsecutiveStonesCoordinates() const {
        ...
        for (int j = 0; j < 19; ++j) {
                ...
                if (board[i][j] == lastStone) {
                    ...
                    if (lastStone == Black) {
                        if (currentCount > maxBlack) {
                            ...
                            maxBlackCoords = { {i - currentCount, j} };
                        }
                        ...
                        if (currentCount > maxWhite) {
                            ...
                            maxWhiteCoords = { {i - currentCount, j} };
                        }
            ...
            if (lastStone == Black) {
                if (currentCount > maxBlack) {
                    maxBlack = currentCount;
                    maxBlackCoords = { {19 - currentCount, j} };
                }
                else if (currentCount == maxBlack) {
                ...
                if (currentCount > maxWhite) {
                    maxWhite = currentCount;
                    maxWhiteCoords = { {19 - currentCount, j} };
                }
                else if (currentCount == maxWhite) {
        ...
        for (int i = 0; i < 19; ++i) {
            ...
            for (int j = 0; j < 19; ++j) {
                if (board[i][j] == lastStone) {
                    ...
                    if (lastStone == Black) {
                        if (currentCount > maxBlack) {
                            maxBlack = currentCount;
                            maxBlackCoords = { {i, j - currentCount} };
                        }
                        else if (currentCount == maxBlack) {
                            maxBlackCoords.push_back({ i, j - currentCount });
                        }
                    }
                    else if (lastStone == White) {
                        if (currentCount > maxWhite) {
                            maxWhite = currentCount;
                            maxWhiteCoords = { {i, j - currentCount} };
                        }
                        else if (currentCount == maxWhite) {
                            maxWhiteCoords.push_back({ i, j - currentCount });
                        }
                    }
                    lastStone = board[i][j];
                    currentCount = 1;
            ...
            if (lastStone == Black) {
                if (currentCount > maxBlack) {
                    maxBlack = currentCount;
                    maxBlackCoords = { {i, 19 - currentCount} };
                }
                else if (currentCount == maxBlack) {
                    maxBlackCoords.push_back({ i, 19 - currentCount });
                }
            }
            else if (lastStone == White) {
                if (currentCount > maxWhite) {
                    maxWhite = currentCount;
                    maxWhiteCoords = { {i, 19 - currentCount} };
                }
                else if (currentCount == maxWhite) {
                    maxWhiteCoords.push_back({ i, 19 - currentCount });
                }
            }
            ...
            for (const auto& coord : maxBlackCoords) {
                for (int k = 0; k < maxBlack; ++k)
                    std::cout << "(" << char('a' + coord.first) << ", " << coord.second + k + 1 << ") ";
            }
            std::cout << "\n  흰돌 최대 연속 (" << maxWhite << "): ";
            for (const auto& coord : maxWhiteCoords) {
                for (int k = 0; k < maxWhite; ++k)
