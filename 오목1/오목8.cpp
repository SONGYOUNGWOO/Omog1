...
public:
    ...
    void placeRandomStone() {
        int x, y;
        do {
            x = rand() % 19; // 0에서 18 사이의 난수 생성
            y = rand() % 19; // 0에서 18 사이의 난수 생성
        } while (!placeStone(y, x)); // 빈 칸에 돌을 놓을 때까지 반복
    }
    ...
    void countDiagonalStones() {
        ...
        for (int i = 0; i < 37; i++) {
            ...
            std::cout << "Diagonal " << i  << ": Black " << diagMajorBlack[i] << ", White " << diagMajorWhite[i] << '\n';
...
int main() {
    ...
    while (true) {
        std::cout << "\n\n\n";
        board.printBoard();
        std::cout << "좌표를 입력하세요(ex 3 5), 종료코드(-1 -1), 랜덤(-2 -2): ";
